...
# FIXME: Add this
import reader as Readers
import model as Models
from trainer import Trainer

from commons.utils import setup_output_dir, read_from_config_file
from commons.model_utils import construct_vocab, load_model_from_existing
...
def main():
    ...
    max_length = config.pop("max_seq_len", None)
    ...
    instances_train = reader.read(file_path=TRAIN_PATH, max_length=max_length)
    ...
    instances_val = reader.read(VAL_PATH, max_length=max_length)
    logger.info("Length of {0}: {1}".format(
    ...
    if TEST_PATH is not None:
        ...
        instances_test = reader.read(TEST_PATH, max_length=max_length)
        logger.info("Length of {0}: {1}".format(
