...
class BasePredictionClass(object):
    ...
    def get_predictions_from_attentions(
        self,
        preds: List[str],
        attns: Dict[str, List[float]]
    ) -> List[str]:
    ...
    def get_predictions(self, instances: List[Instance], model: Model,
        ...
        for batch in inference_generator_tqdm:
            ...
            for ix in range(len(output_dict["preds"])):
                ...
                pred_labels = self.get_predictions_from_attentions(
                    preds=pred,
                    attns=attn
                )
                for p in pred:
        ...
        if prediction_file is not None:
            ...
            if visualization_file is not None and self._visualize:
...
class BasicBinaryPredictions(BasePredictionClass):
    ...
    def get_predictions_from_attentions(
        ...
        tag = preds[0]
        if tag == "O":
            pred_labels = ["O" for _ in range(len(attns[self._tag]))]
        else:
            pred_labels = []
