...
BASE_DIR = os.path.join("Configs/CoNLL-Configs/BERT-realFT-PS")
...
FINETUNE_DICT = {
    "bert-base-multilingual-cased": ["[9, 10, 11]", "[10, 11]", "11"]
}
OPTIMIZER_LIST = [
    # f"""{{
    #             "type": "adam",
...
"""
SCRATCH = "/home/ubuntu/mountdir/AttentionSegmentation"
...
for FINETUNE in FINETUNE_DICT[METHOD]:
    for TLO in TOP_LAYER_ONLY_VALUES:
        ...
        for ix, OPT in enumerate(OPTIMIZER_OPTS):
            ...
            raw = f"""
            {{
              "base_output_dir": "{SCRATCH}/Experiments/CoNLL/BERT-realFT-PS/Dir-{script_no}",
              "dataset_reader": {{
            ...
            config_file = os.path.join(CONF_DIR, f"config_{METHOD}_TLO_{TLO}_FTL_{ft_layers}_OPT_{optstr}.json")
